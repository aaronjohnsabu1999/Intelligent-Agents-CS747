...
algo_dict = {
    "outputDataT1.txt": ["epsilon-greedy", "ucb", "kl-ucb", "thompson-sampling"],
    "outputDataT2.txt": ["thompson-sampling", "thompson-sampling-with-hint"],
}
    ...
    try:
        ...
        if not len_line_ls == number_lines_dict[fname]:
            print(
                "\n",
                "*" * 10,
                "Mistake:number of lines in the output data file should be",
                number_lines_dict[fname],
                "but have ",
                len_line_ls,
                "*" * 10,
                "\n",
        ...
        if not len(set_main) == number_lines_dict[fname]:
            print(
                "\n",
                "*" * 10,
                "Mistake: You didn't print all the combinations. Need ",
                number_lines_dict[fname],
                "but printed ",
                len(set_main),
                "*" * 10,
        ...
        for algo in algo_dict[fname]:
            if not lists[1].count(algo) == 900:
                print(
                    "\n",
                    "*" * 10,
                    "Mistake: Each algorithm should be run 900 times i.e 3 instances times 50 seeds times 6 horizons but ",
                    algo,
                    "has run only",
                    lists[1].count(algo),
                    " times",
        ...
        for i in range(10):
            ...
            if not rep_REG == orig_REG:
                print(
                    "\n",
                    "*" * 10,
                    "Mistake: Unable to reproduce result for ",
                    line_str,
                    " orignal=" + orig_REG + " reproduced=" + rep_REG,
                    "\t",
                    "*" * 10,
                    "\n",
                )
